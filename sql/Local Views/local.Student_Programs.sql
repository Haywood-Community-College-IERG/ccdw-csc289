...
, sp_tn AS (
    ...
    -- Some people do not have a program start date, but do have a catalog year.
      LEFT JOIN dw_dim.Term_CU t
           ON (t.Term_Census_Date >= COALESCE(sp_d.[STPR.START.DATE],
                                          CAST( CASE WHEN COALESCE(sp.[STPR.CATALOG],'')='' THEN '1960'
                                   WHEN sp.[STPR.CATALOG] = 'CNV' THEN '1960'
                             ELSE SUBSTRING(sp.[STPR.CATALOG],1,4) END + '-08-01' AS DATE))
